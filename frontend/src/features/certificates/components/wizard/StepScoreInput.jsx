  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="bg-primary/10 border border-primary/20 rounded-xl p-5 flex items-start gap-4 shadow-sm relative overflow-hidden">
        ...
        <div className="h-12 w-12 bg-card rounded-full flex items-center justify-center border border-primary/20 shadow-sm shrink-0 z-10">
  ...
  return (
    <Card className="border-border bg-card overflow-hidden shadow-sm">
      <div className="p-4 bg-muted border-b border-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto bg-card p-1.5 rounded-lg border border-border shadow-sm">
              ...
              <tr key={student.student_id} className="hover:bg-muted transition-colors group">
                    ...
                    render={({ field, fieldState }) => (
                      <div className="relative">
                        <Select value={field.value || undefined} onValueChange={field.onChange}>
                          <SelectTrigger className={cn("w-full h-10 bg-card font-medium", fieldState.error ? "border-destructive ring-1 ring-destructive/20" : "border-border")}>
                ...
                <td className="px-6 py-4">
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input {...field} value={field.value || ''} placeholder="Ghi chú thêm..." className="h-10 text-sm bg-card border-border/50 focus-visible:border-primary shadow-none" />
                    )}
                  />
                </td>
              </tr>
        ...
        <Card key={student.student_id} className="overflow-hidden border-border bg-card shadow-sm hover:shadow-md transition-shadow relative group">
          ...
          <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-border">
            ...
            <div className="w-full md:w-[35%] shrink-0 p-5 bg-muted">
              ...
              <div className="bg-card p-4 rounded-xl border border-primary/20 shadow-sm relative overflow-hidden group-hover:border-primary/40 transition-colors ml-10">
                  ...
                  render={({ field, fieldState }) => (
                    <div className="relative z-10">
                      <Input 
                        ...
                        className={cn(
                          "bg-card font-bold text-lg h-12 shadow-inner", 
                          ...
                          render={({ field, fieldState }) => (
                            <div>
                              <Input 
                                ...
                                onChange={e => field.onChange(e.target.value ? parseFloat(e.target.value) : undefined)}
                                className={cn(
                                  "h-10 text-base font-semibold bg-card shadow-inner", 
          ...
          <div className="p-5">
            <div className="flex flex-wrap md:flex-nowrap gap-6">
              ...
              <div className="w-full md:w-1/4 shrink-0 flex flex-col justify-center">
                <div className="bg-card p-4 rounded-xl border border-primary/20 shadow-sm relative overflow-hidden group-hover:border-primary/40 transition-colors">
                  ...
                  <div className="flex justify-between items-center mb-2 relative z-10">
                    ...
                    <span className="text-[10px] font-bold bg-card text-primary/70 px-1.5 py-0.5 rounded shadow-sm border border-primary/10">Max: {maxScore || '---'}</span>
                  </div>
                  <Controller
                    control={form.control}
                    ...
                    render={({ field, fieldState }) => (
                      <div className="relative z-10">
                        <Input 
                          ...
                          className={cn(
                            "bg-card font-bold text-lg h-12 shadow-inner pr-12", 
                            fieldState.error ? "border-destructive ring-2 ring-destructive/20 focus-visible:ring-destructive/30" : "border-primary/30 focus-visible:ring-primary/30"
                          )}
                        />
                          ...
                          <Controller
                            ...
                            render={({ field }) => (
                              <Input 
                                ...
                                value={field.value ?? ''}
                                onChange={e => field.onChange(e.target.value ? parseInt(e.target.value, 10) : undefined)}
                                className="h-9 text-sm font-semibold bg-card border-border/70 focus-visible:border-primary/50 shadow-inner"
                              />
                            )}
                          />
                        </div>
