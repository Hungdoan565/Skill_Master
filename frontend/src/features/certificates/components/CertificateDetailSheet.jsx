        ...
        {Object.keys(scores).length > 1 && (
          <div className="grid grid-cols-2 gap-2 mt-1">
            {Object.entries(scores)
              ...
              .map(([key, val]) => (
                <div key={key} className="flex justify-between items-center bg-muted px-2 py-1 rounded text-xs">
        ...
        <div className="relative pt-12 pb-6 px-6 bg-muted/30 border-b border-border/50 overflow-hidden">
          ...
          <SheetHeader className="relative z-10">
            <div className="flex items-start justify-between mb-4">
              <div className="p-3 bg-card rounded-xl shadow-sm border border-border/50 text-primary">
        ...
        <div className="p-6 space-y-8">
                ...
                <div className="px-4">
                  {certificate.external_id && (
                    <InfoRow label="Mã chứng chỉ gốc">
                      <code className="bg-muted px-1.5 py-0.5 rounded text-xs border border-border/50">{certificate.external_id}</code>
                    </InfoRow>
                  )}
          ...
          <div className="space-y-4 pb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {!isExternal && (
                <Button variant="outline" className="w-full bg-card hover:bg-muted border-border/50 shadow-sm" onClick={() => onPrint?.(certificate)}>
                  <Printer className="h-4 w-4 mr-2 text-muted-foreground" /> In / PDF
                </Button>
              )}
              <Button variant="outline" className="w-full bg-card hover:bg-muted border-border/50 shadow-sm" onClick={handleCopyLink}>
                <Link2 className="h-4 w-4 mr-2 text-muted-foreground" /> Copy link
              </Button>
              <Button variant="outline" className={`${isExternal ? 'col-span-2' : 'col-span-2'} w-full bg-card hover:bg-muted border-border/50 shadow-sm`} onClick={() => {
                const url = `${window.location.origin}/verify-certificate?cert=${certificate.certificate_number}`;
