...
export function useStudentSupport() {
  ...
  const fetchTicketDetail = useCallback(async (ticketId) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API}/api/my-support-tickets/${ticketId}`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`
        }
      });
  ...
  const sendReply = useCallback(async (ticketId, message) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API}/api/support-tickets/${ticketId}/messages`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session.access_token}`
      ...
      console.error('Error sending support ticket reply:', err);
