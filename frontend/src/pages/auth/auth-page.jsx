...
const LoginForm = ({ onSwitchToRegister, isAnimating }) => {
  ...
  return (
    <div className={`transition-all duration-500 w-full max-w-[420px] ${isAnimating ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
      <Card className="w-full border-0 bg-white shadow-xl">
...
const LeftPanel = ({ isLogin }) => {
  const loginContent = {
    title1: 'Quản lý đào tạo',
    title2: 'chuyên nghiệp',
    description: 'Nền tảng quản lý toàn diện được tin dùng bởi hơn 100 trung tâm đào tạo tại Việt Nam. Tăng năng suất, tối ưu quy trình làm việc.',
    features: [
      'Quản lý học viên & lớp học',
      'Theo dõi doanh thu & học phí',
      'Báo cáo & phân tích chi tiết',
    ],
  };

  const registerContent = {
    title1: 'Bắt đầu hành trình',
    title2: 'học tập của bạn',
    description: 'Tạo tài khoản miễn phí để khám phá hàng trăm khóa học chất lượng cao và theo dõi tiến độ học tập của bạn.',
    features: [
      'Truy cập hàng trăm khóa học',
  ...
  return (
    <div className="hidden w-1/2 flex-col items-center justify-center bg-slate-900 p-12 lg:flex relative overflow-hidden">
      {/* Animated background gradient */}
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-900/50 via-slate-900 to-purple-900/50 transition-all duration-1000" />
      
      {/* Logo */}
      <div className="absolute top-12 left-12 z-10">
        <a href="/" className="inline-flex items-center gap-3 transition-opacity duration-200 hover:opacity-80">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600">
            <GraduationCap className="h-6 w-6 text-white" />
          </div>
      ...
      {/* Main Content with animation - căn giữa */}
      <div className="space-y-8 max-w-md w-full relative z-10">
        <div className={`transition-all duration-500 ${isLogin ? 'translate-x-0 opacity-100' : '-translate-x-4 opacity-0 absolute'}`}>
          {isLogin && (
            <>
              <h1 className="text-4xl font-bold leading-tight text-white">{loginContent.title1}</h1>
        ...
        <div className={`transition-all duration-500 ${!isLogin ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0 absolute'}`}>
          {!isLogin && (
            <>
              <h1 className="text-4xl font-bold leading-tight text-white">{registerContent.title1}</h1>
              <h2 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">
                {registerContent.title2}
              </h2>
            </>
          )}
        </div>
        
        <p className="text-base text-slate-400 leading-relaxed transition-all duration-500">
          {content.description}
        </p>
