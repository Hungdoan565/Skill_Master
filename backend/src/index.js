...
// Lazy import for triggerCertificateEligibilityCheck (requires PgBoss)
let triggerCertCheckFn = null;
async function getTriggerCertCheck() {
  if (triggerCertCheckFn === null) {
    try {
      const jobs = await import('./jobs/index.js');
  ...
  return triggerCertCheckFn || null;
}
...
const chatbotConversationRateWindowMs = 60 * 1000;
const chatbotConversationRateMax = 30;
const chatbotConversationRateStore = new Map();

function chatbotConversationRateLimit(req, res, next) {
  const actorKey = req.user?.id || req.ip;
  const now = Date.now();
  const windowStart = now - chatbotConversationRateWindowMs;
  const recentHits = (chatbotConversationRateStore.get(actorKey) || []).filter(ts => ts > windowStart);
  ...
  recentHits.push(now);
  chatbotConversationRateStore.set(actorKey, recentHits);

  res.setHeader('X-RateLimit-Limit', String(chatbotConversationRateMax));
  res.setHeader('X-RateLimit-Remaining', String(Math.max(0, chatbotConversationRateMax - recentHits.length)));
...
/**
 * Check if current time is within 24-hour edit window after session ends
 * @param {string} sessionDate - YYYY-MM-DD
 * @param {string} sessionEndTime - HH:MM
 * @returns {Object} { canEdit: boolean, hoursRemaining: number, message: string }
 */
function isWithinEditWindow(sessionDate, sessionEndTime) {
  const sessionEnd = new Date(`${sessionDate}T${sessionEndTime}:00`);
  const now = new Date();
  const hoursSinceEnd = (now - sessionEnd) / (1000 * 60 * 60);
...
app.get('/api/admin/students', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    let eligibleStudentIds = null;

    if (classId || courseId || enrollmentState) {
      ...
      if (effectiveCenterId) {
        enrollmentQuery = enrollmentQuery.eq('classes.center_id', effectiveCenterId);
      }
      if (classId) {
        enrollmentQuery = enrollmentQuery.eq('class_id', classId);
      }
      if (courseId) {
        enrollmentQuery = enrollmentQuery.eq('classes.course_id', courseId);
      }

      if (enrollmentState === 'active') {
        enrollmentQuery = enrollmentQuery.eq('status', 'active');
      } else if (enrollmentState === 'pending') {
        enrollmentQuery = enrollmentQuery.eq('status', 'pending');
      } else if (enrollmentState === 'completed') {
        enrollmentQuery = enrollmentQuery.eq('status', 'completed');
      } else if (enrollmentState === 'inactive') {
        enrollmentQuery = enrollmentQuery.eq('status', 'inactive');
      } else if (enrollmentState === 'has_class' || enrollmentState === 'no_class') {
        enrollmentQuery = enrollmentQuery.in('status', ['active', 'pending', 'completed']);
      }

      const { data: enrollmentRows, error: enrollmentError } = await enrollmentQuery;
      if (enrollmentError) throw enrollmentError;

      const matchedIds = [...new Set((enrollmentRows || []).map((row) => row.student_id).filter(Boolean))];
      eligibleStudentIds = enrollmentState === 'no_class'
        ? { exclude: matchedIds }
        : { include: matchedIds };

      if (!eligibleStudentIds.exclude && (!eligibleStudentIds.include || eligibleStudentIds.include.length === 0)) {
        return res.json({
          success: true,
          data: [],
          pagination: {
    ...
    if (eligibleStudentIds?.include) {
      query = query.in('id', eligibleStudentIds.include);
    }

    if (eligibleStudentIds?.exclude?.length) {
      const quotedIds = eligibleStudentIds.exclude.map((id) => `"${id}"`).join(',');
      query = query.not('id', 'in', `(${quotedIds})`);
    }

    const trimmedSearch = String(search || '').trim();
    if (trimmedSearch) {
      query = query.or(`full_name.ilike.%${trimmedSearch}%,email.ilike.%${trimmedSearch}%,phone.ilike.%${trimmedSearch}%`);
    }
...
app.patch('/api/admin/students/bulk/status', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    let studentsQuery = supabase
      .from('users')
      .select('id, full_name, center_id, roles(code)')
      .in('id', studentIds);

    if (effectiveCenterId) {
      studentsQuery = studentsQuery.eq('center_id', effectiveCenterId);
    }
    ...
    if (status === 'inactive') {
      const { data: activeEnrollments, error: enrollmentError } = await supabase
        .from('enrollments')
        .select('student_id')
        .in('student_id', validStudents.map((student) => student.id))
        .eq('status', 'active');
      if (enrollmentError) throw enrollmentError;

      const blockedStudentIds = new Set((activeEnrollments || []).map((row) => row.student_id));
      const allowedStudents = validStudents.filter((student) => !blockedStudentIds.has(student.id));
      ...
      const { data: updatedStudents, error: updateError } = await supabase
        ...
        .in('id', allowedStudents.map((student) => student.id))
        .select('id, full_name, status');
      if (updateError) throw updateError;
    ...
    const { data: updatedStudents, error: updateError } = await supabase
      ...
      .in('id', validStudents.map((student) => student.id))
      .select('id, full_name, status');
    if (updateError) throw updateError;
    ...
    console.error('Error bulk updating students status:', error);
    next(error);
  }
});

app.post('/api/admin/students/bulk-delete-check', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    const validStudents = (students || []).filter((student) => student.roles?.code === 'STUDENT');
    const validIds = validStudents.map((student) => student.id);

    const [enrollmentsRes, invoicesRes, certificatesRes] = await Promise.all([
      supabase.from('enrollments').select('student_id, status').in('student_id', validIds),
      supabase.from('invoices').select('student_id, status').in('student_id', validIds),
      supabase.from('certificates').select('student_id, status').in('student_id', validIds),
    ]);

    if (enrollmentsRes.error || invoicesRes.error || certificatesRes.error) {
      throw enrollmentsRes.error || invoicesRes.error || certificatesRes.error;
    }

    const blockedReasonsByStudentId = new Map();
    const addReason = (studentId, reason) => {
      const reasons = blockedReasonsByStudentId.get(studentId) || [];
      reasons.push(reason);
    ...
    const blocked = validStudents.filter((student) => blockedReasonsByStudentId.has(student.id)).map((student) => ({
      ...
      reasons: blockedReasonsByStudentId.get(student.id),
    }));

    const allowed = validStudents.filter((student) => !blockedReasonsByStudentId.has(student.id));

    res.json({ success: true, data: { allowed, blocked } });
  } catch (error) {
    console.error('Error checking bulk delete eligibility:', error);
    next(error);
  }
});

app.delete('/api/admin/students/bulk', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    const { studentIds = [] } = req.body;
    ...
    const [enrollmentsRes, invoicesRes, certificatesRes] = await Promise.all([
      supabase.from('enrollments').select('student_id').in('student_id', validIds),
      supabase.from('invoices').select('student_id').in('student_id', validIds),
      supabase.from('certificates').select('student_id').in('student_id', validIds),
    ]);
    ...
    const blockedIds = new Set([
      ...(enrollmentsRes.data || []).map((row) => row.student_id),
      ...(invoicesRes.data || []).map((row) => row.student_id),
      ...(certificatesRes.data || []).map((row) => row.student_id),
    ]);
 ...
 * }
 */
app.post('/api/admin/attendance/override', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    const { sessionId, attendances, override_reason } = req.body;
    ...
    // Get session with class info for center check
    const { data: session, error: sessionError } = await supabase
      .from('sessions')
      .select(`
        id,
    ...
    // CENTER_MANAGER can only override for their center's classes
    ...
    for (const att of attendances) {
      ...
      // Upsert attendance with override fields
      const { data, error } = await supabase
        .from('attendance')
        .upsert({
          session_id: sessionId,
          student_id: att.student_id,
          status: att.status || 'present',
          check_in_time: att.status === 'present' ? now : null,
          notes: att.notes || null,
      ...
      if (error) {
        ...
        // Log to attendance_audit with action='overridden'
        await supabase
...
app.get('/api/admin/attendance/audit/:sessionId', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    // Validate sessionId exists and check center permission for CENTER_MANAGER
    const { data: session, error: sessionError } = await supabase
      .from('sessions')
      .select(`
        id,
    ...
    // CENTER_MANAGER can only view audit for their center's classes
    if (userRole === 'CENTER_MANAGER') {
      const classCenterId = session.classes?.center_id;
      if (classCenterId !== userCenterId) {
    ...
    const { data: auditLogs, error: auditError } = await supabase
      ...
      .select(`
        ...
        ip_address,
        user_agent,
        changed_by_user:users!attendance_audit_changed_by_fkey (
          id,
          full_name,
          email
        ),
        student:users!attendance_audit_student_id_fkey (
          id,
          full_name,
          email
        )
...
app.patch('/api/enrollments/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    if (status === 'completed' && data) {
      try {
        const triggerCertCheck = await getTriggerCertCheck();
        if (triggerCertCheck) {
          await triggerCertCheck({
...
app.get('/api/dashboard/stats', requireAuth, async (req, res, next) => {
  try {
    ...
    if (startDate && endDate) {
      ...
      prevStart = prevPeriodStart.toISOString().split('T')[0];
      prevEnd = prevPeriodEnd.toISOString().split('T')[0];
    } else {
      ...
      prevStart = `${lastMonthYear}-${String(lastMonth).padStart(2, '0')}-01`;
    ...
    let paymentsThisMonthQuery = supabase
      ...
      .gte('payment_date', `${periodStart}T00:00:00`)
      .lte('payment_date', `${periodEnd}T23:59:59`)
      .eq('status', 'completed');

    if (effectiveCenterId) {
      paymentsThisMonthQuery = paymentsThisMonthQuery.eq('classes.center_id', effectiveCenterId);
    }
    ...
    if (effectiveCenterId) {
      enrollmentsDataQuery = enrollmentsDataQuery.eq('classes.center_id', effectiveCenterId);
    }

    const { data: enrollmentsData } = await enrollmentsDataQuery;
    ...
    let paymentsLastMonthQuery = supabase
      .from('payments')
      .select('amount, class:classes!inner(center_id)')
      .gte('payment_date', `${prevStart}T00:00:00`)
      .lte('payment_date', `${prevEnd}T23:59:59`)
      .eq('status', 'completed');

    if (effectiveCenterId) {
      paymentsLastMonthQuery = paymentsLastMonthQuery.eq('classes.center_id', effectiveCenterId);
    }
    ...
    if (effectiveCenterId) {
      newStudentsThisMonthQuery = newStudentsThisMonthQuery.eq('classes.center_id', effectiveCenterId);
    }

    const { count: newStudentsThisMonth } = await newStudentsThisMonthQuery;

    let newStudentsLastMonthQuery = supabase
      .from('enrollments')
      .select('*, classes!inner(center_id)', { count: 'exact', head: true })
    ...
    if (effectiveCenterId) {
      newStudentsLastMonthQuery = newStudentsLastMonthQuery.eq('classes.center_id', effectiveCenterId);
    }

    const { count: newStudentsLastMonth } = await newStudentsLastMonthQuery;
    ...
    if (effectiveCenterId) {
      ongoingClassesQuery = ongoingClassesQuery.eq('center_id', effectiveCenterId);
    }

    const { count: ongoingClasses } = await ongoingClassesQuery;
    ...
    if (effectiveCenterId) {
      upcomingClassesQuery = upcomingClassesQuery.eq('center_id', effectiveCenterId);
    }

    const { count: upcomingClasses } = await upcomingClassesQuery;
    ...
    if (effectiveCenterId) {
      debtDataQuery = debtDataQuery.eq('classes.center_id', effectiveCenterId);
    }

    const { data: debtData } = await debtDataQuery;
    ...
    if (effectiveCenterId) {
      uniqueStudentsQuery = uniqueStudentsQuery.eq('classes.center_id', effectiveCenterId);
    }

    const { data: uniqueStudents } = await uniqueStudentsQuery;
    ...
    res.json({
      ...
      data: {
        revenue: {
          ...
          description: `Doanh thu ${periodStart} - ${periodEnd}`
        },
        newStudents: {
...
app.get('/api/dashboard/revenue-chart', requireAuth, async (req, res, next) => {
  try {
    ...
    if (startDate) {
      ...
      queryStartDate = twelveMonthsAgo.toISOString().split('T')[0];
    ...
    while (currentDate <= endDateObj) {
      ...
      monthlyRevenue[key] = {
        month: `T${currentDate.getMonth() + 1}`,
...
// GET /api/dashboard/teacher-performance - Top teachers with student count
app.get('/api/dashboard/teacher-performance', requireAuth, async (req, res, next) => {
  try {
    ...
    // Query teachers with center info
    let teachersQuery = supabase
      .from('users')
      .select(`
        id, full_name, email, avatar_url, center_id,
        centers!users_center_id_fkey (name)
      `)
      .eq('role_id', teacherRole.id)
      .eq('status', 'active');
    ...
    // Batch query: Get all active classes for these teachers
    ...
    // Batch query: Get all enrollments for these classes
    ...
    if (classIds.length > 0) {
      ...
      (enrollments || []).forEach(e => {
        if (!classStudents[e.class_id]) {
          classStudents[e.class_id] = new Set();
        }
        classStudents[e.class_id].add(e.student_id);
      });

      // Aggregate by teacher
      (allClasses || []).forEach(cls => {
        if (!enrollmentCounts[cls.teacher_id]) {
          enrollmentCounts[cls.teacher_id] = {
            studentIds: new Set(),
            classCount: 0,
            courses: []
          };
        }
        enrollmentCounts[cls.teacher_id].classCount++;
        if (cls.courses?.title) {
          enrollmentCounts[cls.teacher_id].courses.push(cls.courses.title);
        }
        // Add students from this class
        const students = classStudents[cls.id];
        if (students) {
          students.forEach(sid => enrollmentCounts[cls.teacher_id].studentIds.add(sid));
        }
      });
    }
    ...
    const teacherStats = teachers.map(teacher => {
      const stats = enrollmentCounts[teacher.id] || { studentIds: new Set(), classCount: 0, courses: [] };

      // Get most common course as subject
      const courseCounts = {};
...
app.get('/api/dashboard/all', requireAuth, async (req, res, next) => {
  try {
    ...
    const prevStart = prevPeriodStart.toISOString().split('T')[0];
    const prevEnd = prevPeriodEnd.toISOString().split('T')[0];

    const today = now.toISOString().split('T')[0];

    // ============ PARALLEL QUERIES ============
    // Build queries with center filtering at database level
    let enrollmentsQuery = supabase.from('enrollments')
      .select('tuition_fee, paid_amount, student_id, classes!inner(center_id)');

    let enrollmentsThisMonthQuery = supabase.from('enrollments')
      .select('*, classes!inner(center_id)', { count: 'exact', head: true })
      .gte('created_at', `${periodStart}T00:00:00`)
      .lte('created_at', `${periodEnd}T23:59:59`);

    let enrollmentsLastMonthQuery = supabase.from('enrollments')
      .select('*, classes!inner(center_id)', { count: 'exact', head: true })
      .gte('created_at', `${prevStart}T00:00:00`)
      .lte('created_at', `${prevEnd}T23:59:59`);
    ...
    let invoicesQuery = supabase.from('invoices')
      .select('status, final_amount, paid_amount, due_date, class:classes!inner(center_id)')
      .not('status', 'eq', 'cancelled');

    let recentEnrollmentsQuery = supabase.from('enrollments')
      .select(`
        id, created_at, tuition_fee, paid_amount, status,
        users!enrollments_student_id_fkey (id, full_name, email, avatar_url),
        classes!inner (id, code, name, center_id, courses (id, title))
      `)
      .order('created_at', { ascending: false })
      .limit(5);

    let todaySessionsQuery = supabase.from('sessions')
      .select(`
    ...
    if (effectiveCenterId) {
      enrollmentsQuery = enrollmentsQuery.eq('classes.center_id', effectiveCenterId);
      enrollmentsThisMonthQuery = enrollmentsThisMonthQuery.eq('classes.center_id', effectiveCenterId);
      enrollmentsLastMonthQuery = enrollmentsLastMonthQuery.eq('classes.center_id', effectiveCenterId);
      classesQuery = classesQuery.eq('center_id', effectiveCenterId);
      invoicesQuery = invoicesQuery.eq('classes.center_id', effectiveCenterId);
      recentEnrollmentsQuery = recentEnrollmentsQuery.eq('classes.center_id', effectiveCenterId);
      todaySessionsQuery = todaySessionsQuery.eq('classes.center_id', effectiveCenterId);
    }
    ...
    const [
      ...
      enrollmentsQuery,
      enrollmentsThisMonthQuery,
      enrollmentsLastMonthQuery,
      classesQuery,
      invoicesQuery,
      // Courses count (no center filter needed - courses are global)
    ...
    // ============ PROCESS DATA ============
    // Note: Center filtering is now done at database level in queries above
    ...
    // Active classes (already filtered by center at database level)
    const classes = classesResult.data || [];
    const activeClasses = classes.length;

    // Invoice stats (already filtered by center at database level)
    ...
    const recentStudents = (recentEnrollmentsResult.data || []).map(e => {
      ...
      if (e.status === 'cancelled' || e.status === 'dropped') {
        paymentStatus = 'cancelled';
      } else if (tuitionFee > 0 && paidAmount >= tuitionFee) {
        paymentStatus = 'paid';
      } else if (paidAmount > 0 && paidAmount < tuitionFee) {
...
app.get('/api/dashboard/alerts', requireAuth, async (req, res, next) => {
  try {
    ...
    // 4. Certificates Pending Approval
    let pendingCertificatesQuery = supabase
      .from('certificates')
      .select(`
        id,
        certificate_number,
        ...
        class:classes(id, name, center_id),
    ...
    if (effectiveCenterId) {
      pendingCertificatesQuery = pendingCertificatesQuery.eq('center_id', effectiveCenterId);
    }

    const { data: pendingCertificates, error: pendingCertificatesError } = await pendingCertificatesQuery;

    if (pendingCertificatesError) {
      ...
      alerts.certificates_pending = {
        data: (pendingCertificates || []).map((item) => ({
...
app.get('/api/admin/audit-logs', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    const entityType = rawEntityType || legacyEntityType || null;
    const startDate = rawStartDate || legacyStartDate || null;
    const endDate = rawEndDate || legacyEndDate || null;
    ...
    const { effectiveCenterId, error: centerScopeError } = getEffectiveCenterId(req.user, centerId || null);
    if (centerScopeError) {
      return res.status(403).json({ success: false, error: centerScopeError });
    }

    const sanitizedSearch = search.replace(/[%,()']/g, ' ').trim();
    let matchingUserIds = [];

    const applyAuditLogFilters = (baseQuery, { includeMatchedUsers = true } = {}) => {
      let scopedQuery = baseQuery;

      if (effectiveCenterId) scopedQuery = scopedQuery.eq('center_id', effectiveCenterId);
      if (userId) scopedQuery = scopedQuery.eq('user_id', userId);
      if (action) scopedQuery = scopedQuery.eq('action', action);
      if (entityType) scopedQuery = scopedQuery.eq('table_name', entityType);
      if (status) scopedQuery = scopedQuery.eq('status', status);
      if (severity) scopedQuery = scopedQuery.eq('severity', severity);
      if (startDate) scopedQuery = scopedQuery.gte('created_at', startDate);
      if (endDate) scopedQuery = scopedQuery.lte('created_at', endDate);

      if (sanitizedSearch) {
        const searchClauses = [
          `action.ilike.%${sanitizedSearch}%`,
          `table_name.ilike.%${sanitizedSearch}%`,
          `record_id.ilike.%${sanitizedSearch}%`,
          `request_path.ilike.%${sanitizedSearch}%`,
          `user_email.ilike.%${sanitizedSearch}%`,
        ];

        if (includeMatchedUsers && matchingUserIds.length > 0) {
          searchClauses.push(`user_id.in.(${matchingUserIds.join(',')})`);
        }

        scopedQuery = scopedQuery.or(searchClauses.join(','));
      }
    ...
    if (sanitizedSearch) {
      const userSearchQuery = supabase
        .from('users')
        .select('id')
        .or(`full_name.ilike.%${sanitizedSearch}%,email.ilike.%${sanitizedSearch}%`);

      if (effectiveCenterId) {
        userSearchQuery.eq('center_id', effectiveCenterId);
      }

      const { data: matchingUsers, error: userSearchError } = await userSearchQuery.limit(200);
      if (userSearchError) {
        ...
        matchingUserIds = (matchingUsers || []).map((user) => user.id).filter(Boolean);
      }
    }
    ...
    try {
      const query = applyAuditLogFilters(supabase.from('audit_logs').select('*', { count: 'exact' }));
      result = await query.order('created_at', { ascending: false }).range(offset, offset + limit - 1);
      ...
      const runFallbackQuery = async (includeMatchedUsers) => {
        const fallbackQuery = applyAuditLogFilters(
          supabase.from('audit_logs').select('*', { count: 'exact' }),
          { includeMatchedUsers }
        );

        return fallbackQuery.order('created_at', { ascending: false }).range(offset, offset + limit - 1);
      };

      result = await runFallbackQuery(true);
      ...
      if (result.error && sanitizedSearch && matchingUserIds.length > 0) {
        ...
        result = await runFallbackQuery(false);
      }
    ...
    const centerIds = [...new Set(rawLogs.map((log) => log.center_id).filter(Boolean))];

    let usersById = new Map();
    let centersById = new Map();
    ...
    try {
      ...
      if (usersResult.error || centersResult.error) {
        throw usersResult.error || centersResult.error;
      }
    ...
    const logs = rawLogs.map((log) => {
      ...
      const center = centersById.get(log.center_id);
      ...
      return {
        ...
        user_id: log.user_id,
        user: {
          full_name: actor?.full_name || log.user_email || 'System',
          email: actor?.email || log.user_email || null,
          role: log.user_role || null,
        },
        actor: {
          id: log.user_id,
          name: actor?.full_name || log.user_email || 'System',
          role: log.user_role || null,
        },
        action: log.action,
...
/**
 * POST /api/admin/invoices/bulk-payment
...
app.post('/api/admin/invoices/bulk-payment', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    const validMethods = ['cash', 'bank_transfer', 'card', 'momo', 'vnpay'];
    ...
    for (const invoiceId of invoice_ids) {
      try {
        // Get current invoice with remaining amount
        const { data: invoice, error: invoiceError } = await supabase
          .from('invoices')
          .select('id, invoice_code, final_amount, paid_amount, status, student_id')
          .eq('id', invoiceId)
          .single();
        ...
        const verificationStatus = payment_method === 'cash' ? 'verified' : 'pending';
        const autoVerified = payment_method === 'cash';
        ...
        const { data: payment, error: paymentError } = await supabase
          ...
          .insert({
            ...
            payment_method,
            reference_code: reference_code || null,
            notes: notes ? `[Bulk Payment] ${notes}` : '[Bulk Payment]',
            received_by: userId,
            verification_status: verificationStatus,
            verified_by: autoVerified ? userId : null,
            verified_at: autoVerified ? new Date().toISOString() : null
        ...
        // Get updated invoice (trigger should have updated paid_amount and status)
        const { data: updatedInvoice } = await supabase
          .from('invoices')
...
app.get('/api/admin/invoices/overdue', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    if (invoiceIds.length > 0) {
      ...
      if (payments) {
        payments.forEach(p => {
          if (!lastPayments[p.invoice_id]) {
            lastPayments[p.invoice_id] = p.payment_date;
          }
        });
      }
    }
...
app.post('/api/admin/payments/import/parse',
  ...
  async (req, res, next) => {
    try {
      const { fileData, fileType, bankFormat } = req.body;
      ...
      // Import the service
      const { parseBankStatement } = await import('./services/paymentImportService.js');
  ...
  async (req, res, next) => {
    try {
      ...
      let query = supabase
        ...
        .select(`
          *,
          student:users!payment_call_list_student_id_fkey(id, full_name, email, phone),
          invoice:invoices!payment_call_list_invoice_id_fkey(id, invoice_code, final_amount, paid_amount, due_date),
          assigned:users!payment_call_list_assigned_to_fkey(id, full_name)
        `, { count: 'exact' });
...
app.post('/api/admin/call-list',
  ...
  async (req, res, next) => {
    try {
      const { invoiceId, priority = 'normal', notes } = req.body;
      ...
      const { data: invoice, error: invoiceError } = await supabase
        .from('invoices')
        .select('id, student_id, final_amount, paid_amount, due_date')
        .eq('id', invoiceId)
        .single();
      ...
      const amountDue = invoice.final_amount - invoice.paid_amount;
...
app.post('/api/teacher/sessions/:id/attendance', requireAuth, async (req, res, next) => {
  try {
    ...
    const { data: session, error: sessionError } = await supabase
      .from('sessions')
      .select('id, teacher_id, class_id, is_locked, session_date, end_time')
      .eq('id', sessionId)
      .single();
...
/**
 * GET /api/teacher/sessions/:id/edit-status
 * Check if teacher can edit attendance for this session
 */
app.get('/api/teacher/sessions/:id/edit-status', requireAuth, async (req, res, next) => {
  try {
    const teacherId = req.user.id;
    const { id: sessionId } = req.params;
    ...
    const { data: session, error: sessionError } = await supabase
      .from('sessions')
      .select('id, teacher_id, class_id, is_locked, session_date, end_time, status')
      .eq('id', sessionId)
      .single();
    ...
    // Determine final canEdit status (must pass both edit window AND not locked)
    const canEdit = editWindow.canEdit && !session.is_locked;
    ...
    res.json({
      ...
      data: {
        canEdit,
        hoursRemaining: editWindow.hoursRemaining,
...
app.get('/api/teacher/classes/:id/grades', requireAuth, async (req, res, next) => {
  try {
    ...
    const { data: classData, error: classError } = await supabase
      .from('classes')
      .select('id, name, code, teacher_id, course_id, center_id')
      .eq('id', classId)
      .single();
    ...
    const { data: gradeStructures, error: structError } = await supabase
      .from('grade_structures')
      .select('id, name, weight, max_score, order_index, description')
      .eq('course_id', classData.course_id)
      .order('order_index', { ascending: true });
    ...
    let gradesQuery = supabase
      .from('grades')
      .select('id, enrollment_id, grade_structure_id, score, notes, graded_by, graded_at, is_locked, locked_at, locked_by')
      .in('enrollment_id', enrollments?.map(e => e.id) || []);

    if (gradeStructureId) {
      gradesQuery = gradesQuery.eq('grade_structure_id', gradeStructureId);
    }
    ...
    const studentsWithGrades = enrollments?.map(enrollment => {
      ...
      studentGrades.forEach(g => {
        const structure = gradeStructures?.find(s => s.id === g.grade_structure_id);
        if (g.score !== null && structure) {
          const normalized = (g.score / structure.max_score) * 10;
          weightedSum += normalized * structure.weight;
          totalWeight += structure.weight;
        }
      });

      const finalGrade = totalWeight > 0 ? (weightedSum / totalWeight).toFixed(2) : null;
      ...
      return {
        ...
        grades: studentGrades,
        finalGrade: finalGrade ? parseFloat(finalGrade) : null
      };
    }) || [];
    ...
    if (studentId) {
      filteredStudents = studentsWithGrades.filter(s => s.student_id === studentId);
    }

    // Calculate class summary
    const allFinalGrades = studentsWithGrades
      .map(s => s.finalGrade)
      .filter(g => g !== null);

    const summary = {
      totalStudents: studentsWithGrades.length,
      gradedStudents: allFinalGrades.length,
      classAverage: allFinalGrades.length > 0
        ? (allFinalGrades.reduce((a, b) => a + b, 0) / allFinalGrades.length).toFixed(2)
        : null,
      highest: allFinalGrades.length > 0 ? Math.max(...allFinalGrades) : null,
      lowest: allFinalGrades.length > 0 ? Math.min(...allFinalGrades) : null
    };

    // Check lock status per grade structure
    const lockStatus = {};
    gradeStructures?.forEach(gs => {
      const structureGrades = grades?.filter(g => g.grade_structure_id === gs.id) || [];
      const lockedGrade = structureGrades.find(g => g.is_locked);
      lockStatus[gs.id] = {
        isLocked: !!lockedGrade,
        lockedAt: lockedGrade?.locked_at,
        lockedBy: lockedGrade?.locked_by
      };
    });
...
app.post('/api/teacher/classes/:id/grades', requireAuth, async (req, res, next) => {
  try {
    ...
    const { data: gradeStructure, error: structError } = await supabase
      .from('grade_structures')
      .select('id, name, max_score, course_id')
      .eq('id', gradeStructureId)
      .single();
    ...
    const { data: existingGrades } = await supabase
      ...
      .in('enrollment_id', grades.map(g => g.enrollment_id));
    ...
    const upsertData = grades.map(g => ({
      ...
      score: g.score !== undefined && g.score !== '' ? parseFloat(g.score) : null,
      notes: g.notes || null,
      graded_by: teacherId,
      graded_at: new Date().toISOString()
    }));

    const { data: savedGrades, error: saveError } = await supabase
      .from('grades')
      .upsert(upsertData, {
        onConflict: 'enrollment_id,grade_structure_id',
        ignoreDuplicates: false
...
app.get('/api/teacher/classes/:id/grades/summary', requireAuth, async (req, res, next) => {
  try {
    ...
    const { data: classData, error: classError } = await supabase
      .from('classes')
      .select('id, name, teacher_id, course_id')
      .eq('id', classId)
      .single();
    ...
    const { data: grades } = await supabase
      .from('grades')
      .select('enrollment_id, grade_structure_id, score')
      .in('enrollment_id', enrollments?.map(e => e.id) || []);

    // Calculate stats per grade structure
    const structureStats = gradeStructures?.map(gs => {
      const structureGrades = grades?.filter(g => g.grade_structure_id === gs.id && g.score !== null) || [];
      const scores = structureGrades.map(g => g.score);
      ...
      return {
        ...
        count: scores.length,
        average: scores.length > 0 ? (scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(2) : null,
        highest: scores.length > 0 ? Math.max(...scores) : null,
        lowest: scores.length > 0 ? Math.min(...scores) : null
      };
    }) || [];

    // Calculate final grades for distribution
    const finalGrades = enrollments?.map(enrollment => {
      let totalWeight = 0;
      let weightedSum = 0;

      gradeStructures?.forEach(gs => {
        const grade = grades?.find(g => g.enrollment_id === enrollment.id && g.grade_structure_id === gs.id);
        if (grade?.score !== null && grade?.score !== undefined) {
          const normalized = (grade.score / gs.max_score) * 10;
          weightedSum += normalized * gs.weight;
          totalWeight += gs.weight;
        }
      });
    ...
    // Grade distribution (A: 9-10, B: 8-9, C: 7-8, D: 5-7, F: <5)
    const distribution = {
      A: finalGrades.filter(g => g >= 9).length,
      B: finalGrades.filter(g => g >= 8 && g < 9).length,
      C: finalGrades.filter(g => g >= 7 && g < 8).length,
      D: finalGrades.filter(g => g >= 5 && g < 7).length,
      F: finalGrades.filter(g => g < 5).length
    };
    ...
    res.json({
      ...
      data: {
        ...
        classAverage: finalGrades.length > 0
          ...
          : null,
        highest: finalGrades.length > 0 ? Math.max(...finalGrades).toFixed(2) : null,
        lowest: finalGrades.length > 0 ? Math.min(...finalGrades).toFixed(2) : null,
        passRate: finalGrades.length > 0
          ? ((finalGrades.filter(g => g >= 5).length / finalGrades.length) * 100).toFixed(1)
          : null,
        structureStats,
        distribution
      }
...
app.post('/api/teacher/classes/:id/grades/lock', requireAuth, async (req, res, next) => {
  try {
    ...
    // Update grades to locked
    const { data: lockedGrades, error: lockError } = await supabase
      .from('grades')
      .update({
        is_locked: true,
...
app.post('/api/admin/grades/unlock', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    // Update grades to unlocked
    const { data: unlockedGrades, error: unlockError } = await supabase
      .from('grades')
      .update({
        is_locked: false,
        locked_at: null,
    ...
    // Log to audit
    await supabase.from('grade_audit').insert({
      class_id: classId,
      grade_structure_id: gradeStructureId,
      action: 'unlocked',
      changed_by: userId,
...
app.post('/api/admin/grades/override', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    const { data: oldGrades } = await supabase
      .from('grades')
      .select('*')
      .eq('grade_structure_id', gradeStructureId)
      .in('enrollment_id', grades.map(g => g.enrollment_id));

    // Upsert grades with override info
    const upsertData = grades.map(g => ({
      enrollment_id: g.enrollment_id,
      grade_structure_id: gradeStructureId,
      score: g.score !== undefined && g.score !== '' ? parseFloat(g.score) : null,
...
app.get('/api/admin/certificates/stats', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER', 'TEACHER']), async (req, res, next) => {
  try {
    ...
    const today = new Date().toISOString().split('T')[0];
    const expiringSoonDate = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];

    const buildCountQuery = () => {
      let query = supabase.from('certificates').select('id', { count: 'exact', head: true });
      if (effectiveCenterId) {
        query = query.eq('center_id', effectiveCenterId);
      }
      return query;
    ...
    try {
      ...
      } else if (pendingError.code !== '42P01') {
        throw pendingError;
      }
    } catch (pendingError) {
    ...
    const [
      ...
      buildCountQuery().eq('status', 'issued'),
      buildCountQuery().eq('status', 'issued').or(`expires_at.is.null,expires_at.gte.${today}`),
      buildCountQuery().eq('status', 'issued').gte('expires_at', today).lte('expires_at', expiringSoonDate),
      buildCountQuery().eq('status', 'revoked'),
      (() => {
        let query = supabase
          .from('certificates')
          .select('id, certificate_number, student_name, course_name, issued_at, status, certificate_type:certificate_types(id, code, name), center_id')
          .order('issued_at', { ascending: false })
          .limit(5);
      ...
      (() => {
        let query = supabase
          .from('certificates')
          .select('certificate_type_id, center_id')
          .not('certificate_type_id', 'is', null);
    ...
    const firstError = [
      totalIssuedRes.error,
      activeCountRes.error,
      expiringSoonRes.error,
      revokedCountRes.error,
      recentCertificatesRes.error,
      topTypesRawRes.error,
    ].find(Boolean);

    if (firstError) throw firstError;

    const topTypeIds = [...new Set((topTypesRawRes.data || []).map((item) => item.certificate_type_id).filter(Boolean))];
    const { data: typeMeta, error: typeMetaError } = topTypeIds.length > 0
      ? await supabase
        .from('certificate_types')
    ...
    if (typeMetaError) throw typeMetaError;

    const typeMetaMap = new Map((typeMeta || []).map((item) => [item.id, item]));
    const typeCounter = {};

    for (const row of topTypesRawRes.data || []) {
      if (!row.certificate_type_id) continue;
      typeCounter[row.certificate_type_id] = (typeCounter[row.certificate_type_id] || 0) + 1;
    }
    ...
    return res.json({
      success: true,
      data: {
        total_issued: totalIssuedRes.count || 0,
        active_count: activeCountRes.count || 0,
        expiring_soon: expiringSoonRes.count || 0,
        revoked_count: revokedCountRes.count || 0,
        pending_approval_count: pendingApprovalCount,
        recent_certificates: recentCertificatesRes.data || [],
        top_types: topTypes,
      },
    });
  } catch (error) {
...
app.post('/api/admin/certificates', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    if (req.body.is_external) {
      const { external_id, exam_date, scores, external_verify_url, file_url, notes } = req.body;
      ...
      const { effectiveCenterId: centerId, error: permError } = getEffectiveCenterId(req.user, null);
      if (permError) {
        return res.status(403).json({
          success: false,
          message: permError
      ...
      const { data: certificateType, error: certTypeError } = await supabase
        .from('certificate_types')
        .select('name')
      ...
      const { data: certificateNumber, error: certificateNumberError } = await supabase
      ...
      const { data: certificate, error: insertError } = await supabase
        ...
        .insert({
          ...
          course_name: certificateType.name,
          certificate_type_id,
          scores: scores || {},
...
app.get('/api/admin/certificates/eligible-students', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    const certificateTypeId = certTypeIdParam || type_id;
    const { effectiveCenterId, error: permError } = getEffectiveCenterId(req.user, centerId);
    if (permError) {
      return res.status(403).json({ success: false, message: permError });
    }
    ...
    // Filter by center
    let enrollments = (completedEnrollments || []).filter(e => {
    ...
    const existingCertKeys = new Set(
      (existingCerts || []).map(c => `${c.student_id}_${c.class_id}_${c.certificate_type_id || ''}`)
    );

    // Check eligibility using DB function for each student
    const eligibleStudents = [];
    for (const enrollment of enrollments) {
      // Skip if already has certificate for this type+class combo
      if (certificateTypeId) {
        const key = `${enrollment.student_id}_${enrollment.class?.id}_${certificateTypeId}`;
        if (existingCertKeys.has(key)) continue;
      }

      // Check eligibility via service function (uses DB function)
      if (certificateTypeId) {
        const eligibility = await checkCertificateEligibility(
          supabase, enrollment.student_id, enrollment.class_id, certificateTypeId
        );
        eligibleStudents.push({
          ...
          course_name: enrollment.class?.course?.title || '',
          is_eligible: eligibility.eligible,
          eligible: eligibility.eligible,
          attendance_rate: eligibility.attendance_rate,
          average_grade: eligibility.average_grade,
          min_attendance_required: eligibility.min_attendance_required,
          min_grade_required: eligibility.min_grade_required,
...
app.get('/api/admin/certificates/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    if (id === 'pending-approvals' || id === 'cert_pending_approvals') {
      return next('route');
    }
...
const listPendingCertificateApprovals = async (req, res, next) => {
  try {
    ...
    const buildPendingCertificatesSelect = (includeTypeRelation = true) => `
        id,
        certificate_number,
        certificate_type_id,
        status,
        ...
        course:courses(id, code, title)
        ${includeTypeRelation ? ',\n        certificate_type_ref:certificate_types!certificates_certificate_type_id_fkey(id, code, name)' : ''}
      `;

    const relationStrategies = [true, false];
    let data = null;
    let error = null;

    for (const includeTypeRelation of relationStrategies) {
      let query = supabaseClient
        .from('certificates')
        .select(buildPendingCertificatesSelect(includeTypeRelation))
        .eq('status', 'pending_approval')
        .order('created_at', { ascending: false });
...
app.get('/api/admin/certificates/pending-approvals', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), listPendingCertificateApprovals);

// Backward-compatible alias for older frontend bundles
app.get('/api/admin/cert_pending_approvals', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), listPendingCertificateApprovals);
...
app.put('/api/admin/certificates/:id/approve', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    const id = typeof rawId === 'string' && rawId.startsWith('cert_') ? rawId.slice(5) : rawId;
...
app.get('/api/admin/consultation-requests', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER', 'TEACHER']), async (req, res, next) => {
  try {
    const { status, source, search, assigned_to, center_id: queryCenterId, page = 1, limit = 50 } = req.query;
    const { effectiveCenterId, error: permError } = getEffectiveCenterId(req.user, queryCenterId);
...
app.put('/api/admin/consultation-requests/:id', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER', 'TEACHER']), async (req, res, next) => {
  try {
    ...
    if (status !== undefined) {
      ...
      if (status === 'assigned' && !existingRequest.assigned_at) {
        updateData.assigned_at = nowIso;
      }
      if (status === 'contacted' && !existingRequest.contacted_at) {
        updateData.contacted_at = nowIso;
      }
      if (status === 'scheduled' && !existingRequest.scheduled_at) {
        updateData.scheduled_at = nowIso;
      }
      if ((status === 'closed' || status === 'lost') && !existingRequest.closed_at) {
    ...
    if (assigned_to !== undefined) {
      updateData.assigned_to = assigned_to || null;
      updateData.assigned_at = assigned_to ? nowIso : null;

      if (assigned_to && existingRequest.status === 'new' && status === undefined) {
    ...
    if (handoff_reason !== undefined) {
      updateData.handoff_reason = handoff_reason?.trim() || null;
    }
  ...
  async (req, res, next) => {
    try {
      ...
      for (const grade of grades) {
        ...
        classStats[cid].grades.push(grade);
        classStats[cid].totalScore += (grade.score || 0) * (grade.weight || 1);
        classStats[cid].totalWeight += (grade.weight || 1);
      }

      // Calculate weighted average per class
      const classSummaries = Object.values(classStats).map(cs => ({
        ...cs,
        avgScore: cs.totalWeight > 0 ? (cs.totalScore / cs.totalWeight).toFixed(2) : null,
        gradeCount: cs.grades.length
      }));
  ...
  async (req, res, next) => {
    try {
      ...
      let query = supabase
        .from('attendance')
        .select(`
          id, status, notes, check_in_time, created_at,
          session:sessions(
            id, session_date, start_time, end_time,
            class:classes(id, name, code, course:courses(id, title))
          ),
      ...
      // Filter by date range
      if (startDate) {
        filtered = filtered.filter(a => a.session?.session_date >= startDate);
      }
      if (endDate) {
        filtered = filtered.filter(a => a.session?.session_date <= endDate);
      }
      ...
      for (const record of filtered) {
        ...
        classAttendance[cid].total++;
        classAttendance[cid][record.status]++;
      }
...
app.get('/api/admin/leave-requests', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    const buildLeaveRequestsSelect = (relationAlias) => `
        id,
        leave_type,
        start_date,
        end_date,
        ...
        created_at,
        teacher:users!${relationAlias}(id, full_name, email)
      `;

    const relationAliases = ['leave_requests_staff_id_fkey', 'leave_requests_teacher_id_fkey'];
    let data = null;
    let error = null;
    let count = 0;

    for (const relationAlias of relationAliases) {
      let query = supabaseClient
        .from('leave_requests')
        .select(buildLeaveRequestsSelect(relationAlias), { count: 'exact' })
        .order('created_at', { ascending: false })
        .range(offset, offset + parseInt(limit) - 1);
...
app.patch('/api/admin/leave-requests/:id', requireAuth, requireRole(['SUPER_ADMIN', 'CENTER_MANAGER']), async (req, res, next) => {
  try {
    ...
    const { data, error } = await supabaseClient
      ...
      .update({
        status,
        reviewer_notes: admin_note || null,
        reviewed_by: req.user.id,
        reviewed_at: new Date().toISOString()
      })
      .eq('id', id)
...
app.get('/api/chatbot/messages/:sessionId', async (req, res, next) => {
  try {
    ...
    const { data: session, error: sessionError } = await supabase
      .from('chat_sessions')
      .select('id, user_id, visitor_id, deleted_at')
      .eq('id', sessionId)
      .maybeSingle();
...
function compactLeadText(text, maxLength = 200) {
...
function inferLeadUrgency(primaryNeed, userMessages = []) {
  ...
  if (warmSignals.some(keyword => corpus.includes(keyword))) {
    return 'warm';
  }
  return 'cold';
}
...
function buildChatbotHandoffContext(recentMessages, { preferredTimeLabel, pageContext }) {
  ...
  const userMessages = chronological
    .filter(message => message.role === 'user')
    .map(message => compactLeadText(message.content, 220))
    .filter(Boolean);
  ...
  return {
    transcriptSummary: advisorBriefLines.join('\n'),
  ...
  try {
    ...
    if (sessionId) {
      ...
      if (recentMessages?.length) {
        const handoffContext = buildChatbotHandoffContext(recentMessages, {
          preferredTimeLabel: preferredTime ? timeSlotMap[preferredTime] : null,
          pageContext
        });
        transcriptSummary = handoffContext.transcriptSummary;
        transcriptExcerpt = handoffContext.transcriptExcerpt;
        primaryNeed = handoffContext.primaryNeed;
        summaryLine = handoffContext.summaryLine;
        urgencyLevel = handoffContext.urgencyLevel;
      }
    }

    const handoffReason = 'chatbot_consultation_request';
    const metadata = {
      preferred_time_key: preferredTime || null,
      source_page: pageContext || null,
      primary_need: primaryNeed,
      summary_line: summaryLine,
      urgency_level: urgencyLevel,
      advisor_brief: transcriptSummary,
...
app.get('/api/chatbot/conversations', requireAuth, chatbotConversationRateLimit, async (req, res, next) => {
  try {
    const userId = req.user.id;
...
app.post('/api/chatbot/conversations', requireAuth, chatbotConversationRateLimit, async (req, res, next) => {
  try {
    const userId = req.user.id;
...
app.patch('/api/chatbot/conversations/:id', requireAuth, chatbotConversationRateLimit, async (req, res, next) => {
  try {
    const { id } = req.params;
    const userId = req.user.id;
...
app.delete('/api/chatbot/conversations/:id', requireAuth, chatbotConversationRateLimit, async (req, res, next) => {
  try {
    const { id } = req.params;
    const userId = req.user.id;
...
app.post('/api/chatbot/messages/:id/rate', requireAuth, chatbotConversationRateLimit, async (req, res, next) => {
  try {
    const { id } = req.params;
    const userId = req.user.id;
    const { rating } = req.body;
    const normalizedRating = rating === 'like'
      ? 'up'
      : rating === 'dislike'
        ? 'down'
        : rating;
    ...
    const { error: updateError } = await supabase
      .from('chat_messages')
      .update({ rating: normalizedRating || null })
      .eq('id', id);
...
// Ensure errors from routes defined late in this file also return JSON (not default HTML)
app.use((err, _req, res, _next) => {
